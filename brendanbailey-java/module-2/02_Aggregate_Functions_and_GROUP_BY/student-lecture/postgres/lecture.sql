...
--
-- The database manager does not gurantee the order of the rows in the result unless you have an ORDER BY
--
-- The same query without an ORDER BY may return the same order results 100 times in a row, the 101st time could be different
--
-- If the order of the rows in the result matter - code an ORDER BY - DO NOT ASSUME THE ORDER IS ALWAYS RIGHT 
--
...
select name, population
        from country
        order by Population asc -- (low-high)        
;

-- Show Populations of all countries in descending order
select name, population
        from country
        order by Population desc -- (low-high)        
;
...
;
-- Show population density for each country from highest to lowest
select name , population / surfacearea  -- derived column does not have a name
from country
order by population_density -- order by column
;

-- Show population density for each country from highest to lowest
select name 
        , population / surfacearea as population_density -- derived column can be assigned a name
--      ,population
--      ,surfacearea
from country
order by population_density -- order using derived name
;
--------------------------------------------------------------------------------------------------------
...
-- 
-- LIMIT is a postgreSQL word - all databases have a way to LIMIT results - might be called something else
--
...
select distinct name, lifeexpectancy    -- distinct is applied to all columns in the select
        from country
        where lifeexpetancy is not null
        order by lifeexpectancy desc
limit 10
;
--
-- List all the unique continent names
--
--
select continent -- shows all continents with duplicates
from country
;
-- 
--
--
select distinct contintent -- list all unique continent names
from country
;
--
--
...
from country
;                          -- no group by specified so one value for the entire table is generated
 
-- Show average life expectancy of each continent
select continent, avg(lifeexpectancy) -- avg of non-null lifeexpectancy
from country
where lifeexpectancy is not null      -- dont include any rows if lifeexpectancy is unknown
...
select sum(population) as Ohio_Population -- use (as) to make things more descriptive
   from city
where district = 'Ohio'
;
