...
import org.junit.*;
import org.junit.runners.MethodSorters; // This is needed for @FixMethodOrder to work
import static org.junit.Assert.*;		// Allow us to no have to code Assert.

// 04_Arrays Tests

@FixMethodOrder(MethodSorters.NAME_ASCENDING)
public class LectureTest {
	// Define an object containing the methods to test
    Lecture exercises = new Lecture(); // instantiate object with methods to test

    @Test
    public void testReturnArray() {	// test to see if an array contains specific values
    	// combined Arrange, Act, Assert in one statement							
    					// create an array for the test, use testObject to run method
    ...
    //	Because double to decimal conversion is inexact, the fudge factor is a margin of error for the result    
    }
