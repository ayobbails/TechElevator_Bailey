...
public class App {
	...
	//static - only one defined
	private static final String API_BASE_URL = "http://localhost:3000/"; 
	//server can change and we wont have to change it in more than one place
	//loosely coupled with the constant	 

	// Define a RestTemplate to make our API calls
	//Framework API Interface object		
	...
	private static void run() {
		...
		int hotelWeWant = 1; // Somehow you get a value for the hotel id you want (from a database, user keyboard, calculation)
		int starsWeWant = 3;
		printGreeting();
		...
		while(menuSelection != 0) {
			...
			if (menuSelection == 1) {
				...
				// 									  Tell RestTemplate we want an array of Hotel obects returned	
				...
				printHotels (restTemplate.getForObject  (API_BASE_URL + "hotels?stars=" + starsWeWant,    Hotel[].class));
