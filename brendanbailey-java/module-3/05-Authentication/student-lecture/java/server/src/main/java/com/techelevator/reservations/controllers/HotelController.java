 ...
 *****************
 **/
@PreAuthorize("isAuthenticated()") // Ask Spring to check if whoever wants to us this method is authorized
@RestController
public class HotelController {
     ...
     */
    @PreAuthorize("permitAll") // Allow access to this method (path) without authorization
    ...
    // AUTHORIZATION - role based, Only allow users who are also Adminstrator the ability to delete a reservation
    // An Adminstrator is a user with the role "ADMIN"
    ...
    // Spring Security has a special class called Principal that in effect represents the user (includes things that arent user related)
    // YOU dont have to do anything except declare it as a parameter to a method
    
    // the getName() method of the principal class returns the name of the currently logged in user
    
    public void delete(@PathVariable int id, Principal thePrincipal) throws ReservationNotFoundException {
    	
        auditLog("delete", id, thePrincipal.getName()); // Log the operation as a delete for the id by the current user
